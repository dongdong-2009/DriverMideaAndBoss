    ...
    public class Code
    {
        ...
        public class Region
        {
            ...
            public static ErrorList isRegion(string LocalCode, string Code, out string ErrorStr)
            {
                ...
                string[] CureValue = Decryption(LocalCode, DateTime.Now).Split('-');
                if (ReadValue.Length != CureValue.Length || ReadValue.Length < 6)
                {
                    ErrorStr = "本地码或注册码格式不正确";
            ...
            /// <summary>
            /// 生成一个长度为4的字符串,代表注册日期,只能表示1950-1-1 到 2068-12-31 128年时间
            ...
            {
                DateTime result = new DateTime(1951, 1, 1);
                try
                {
                    int A = Convert.ToInt16(value.Substring(0, 1), 16);
                    int B = Convert.ToInt16(value.Substring(1, 1), 16);
                    int C = Convert.ToInt16(value.Substring(2, 1), 16);
                    int D = Convert.ToInt16(value.Substring(3, 1), 16);
                    int year = (A << 3) + (B >> 1) + 1951;
