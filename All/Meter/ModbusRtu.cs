...
namespace All.Meter
{
    public class ModbusRtu:Meter
    {
        ...
        public override bool Read<T>(out List<T> value, Dictionary<string, string> parm)
        {
            lock (lockObject)
            {
                ...
                try
                {
                    ...
                    sendBuff[7] = crcHi;
                    byte[] readBuff;
                    if (Write<byte[], byte[]>(sendBuff, returnCount, out readBuff))
                    {
                        if (sendBuff[0] != readBuff[0])
                        {
                            All.Class.Log.Add("返回数据地址与读取地址不一致", Environment.StackTrace);
                            return false;
                        }
                        if (sendBuff[1] != readBuff[1])
                        {
                            All.Class.Log.Add("返回数据指令码与读取指令码不一致", Environment.StackTrace);
                            return false;
                        }
                        ...
                        {
                            All.Class.Log.Add("返回数据CRC校验失败", Environment.StackTrace);
                            return false;
                        }
                            ...
                            case All.Class.TypeUse.TypeList.Boolean:
                                switch (readBuff[1])
                                {
                                    case 1:
                                        ...
                                        break;
                                    default:
                                        All.Class.Log.Add("当前使用的读取指令不能将数据转化bool");
                                        result = false;
                                        break;
                                }
                                break;
                            case All.Class.TypeUse.TypeList.UShort:
                                switch (readBuff[1])
                                {
                                    case 3:
                                        ...
                                        break;
                                    default:
                                        All.Class.Log.Add("当前使用的读取指令不能将数据转化Ushort");
                                        result = false;
                                        break;
                                }
                                break;
        ...
        public override bool WriteInternal<T>(List<T> value, int start, int end)
        {
            ...
            parm.Add("Code", "16");
            int len = end - start + 1;
            int tmpCount = value.Count;
            if (tmpCount > len)
            {
                for (int i = len; i < tmpCount; i++)
                {
                    value.RemoveAt(len);
                }
            }
        ...
        public override bool WriteInternal<T>(List<T> value, Dictionary<string, string> parm)
        {
            lock (lockObject)
            {
                ...
                try
                {
                    ...
                    if (!parm.ContainsKey("Code"))
                    {
                        All.Class.Error.Add("数据读取参数中不包含操作指令", Environment.StackTrace);
                        return false;
                    }
                    else
                    {
