...
namespace All.Meter
{
    public class Simens1200Net : Meter
    {
                ...
                try
                {
                    ...
                    switch (Class.TypeUse.GetType<T>())
                    {
                        ...
                        case All.Class.TypeUse.TypeList.String:
                            ...
                            sendBuff[27] = 0x84;
                            sendBuff[28] = (byte)((((int)start * 8) >> 0x10) & 0xFF);
                            sendBuff[29] = (byte)((((int)start * 8) >> 0x08) & 0xFF);
                            sendBuff[30] = (byte)((((int)start * 8) >> 0x00) & 0xFF);
                        ...
                        case All.Class.TypeUse.TypeList.Boolean:
                            ...
                            sendBuff[21] = 0x10;
                            sendBuff[22] = 0x02;
                            sendBuff[23] = (byte)((len >> 8) & 0xFF);
                            sendBuff[24] = (byte)((len >> 0) & 0xFF);
                            sendBuff[25] = (byte)((((int)Block) >> 8) & 0xFF);
                            sendBuff[26] = (byte)((((int)Block) >> 0) & 0xFF);
                            sendBuff[27] = 0x83;
                            sendBuff[28] = (byte)(((tmpStart * 8) >> 0x10) & 0xFF);
                            sendBuff[29] = (byte)(((tmpStart * 8) >> 0x08) & 0xFF);
                            sendBuff[30] = (byte)(((tmpStart * 8) >> 0x00) & 0xFF);
                            readLen = 25 + len;
                            break;
                        default:
                    ...
                    if (Write<byte[], byte[]>(sendBuff, readLen, out readBuff))
                    {
                        if (readBuff[0] == 0x03 && readLen >= 22 && readBuff[12] == sendBuff[12] && readBuff[21] == 0xFF && ((readBuff[23] * 0x100 + readBuff[24]) / 8 + 25) == readLen)
                        {
                            ...
                            {
                                case All.Class.TypeUse.TypeList.Byte:
                                    for (int i = 25; i < (readBuff[23] * 0x100 + readBuff[24]) / 8 && i < readBuff.Length; i++)
                                    {
                                ...
                                case All.Class.TypeUse.TypeList.String:
                                    if (parm.ContainsKey("Part") && All.Class.Num.ToInt(parm["Part"]) > 0)
                                    {
                                        ...
                                        for (int i = 0; i < (readBuff[23] * 0x100 + readBuff[24]) / 8; i = i + part)
                                        {
                                            value.Add((T)(object)Encoding.ASCII.GetString(readBuff, 25 + i, part));
                                        }
                                    }
                                    else
                                    {
                                    ...
                                    break;
                                case All.Class.TypeUse.TypeList.Boolean:
                                    bool[] tmpBool = All.Class.Num.Byte2Bool(readBuff, 25, len);
                                    int startBit = Convert.ToInt16((start * 10 - tmpStart * 10));
                                    int endBit = Convert.ToInt16((end * 10 - tmpEnd * 10));
                                    for (int i = startBit; i < tmpBool.Length && i < (len - 1) * 8 + endBit + 1; i++)
                                    {
                ...
                try
                {
                    ...
                    switch (Class.TypeUse.GetType<T>())
                    {
                        ...
                        case All.Class.TypeUse.TypeList.Byte:
                            ...
                            break;
                        case All.Class.TypeUse.TypeList.Boolean:
                            buff = new byte[1];//写多个M点没有研究出来，累啊
                            if ((bool)(object)value[0])
                            {
                                buff[0] = 1;
                            }
                            else
                            ...
                            tmpStart = (int)Math.Floor(start);
                            startBit = Convert.ToInt16((start * 10 - tmpStart * 10));
                            sendBuff = new byte[0x23 + buff.Length];
                            sendBuff[22] = 0x01;
                            sendBuff[27] = 0x83;
                            sendBuff[32] = 0x03;
                    ...
                    sendBuff[28] = (byte)((((int)start * 8 + startBit) >> 0x10) & 0xFF);
                    sendBuff[29] = (byte)((((int)start * 8 + startBit) >> 0x08) & 0xFF);
                    sendBuff[30] = (byte)((((int)start * 8 + startBit) >> 0x00) & 0xFF);
                    sendBuff[31] = 0x00;
